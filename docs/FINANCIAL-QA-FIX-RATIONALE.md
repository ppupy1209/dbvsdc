...
5. 미래 시뮬레이션을 평균수익률 직선 투영에서 보수적 스트레스 경로로 바꿨습니다.
   - 기존 미래 모드: 선택 지수의 전체 기간 CAGR을 매년 반복
   - 변경 미래 모드: 동일 기간의 과거 모든 구간 중 DC 최종 잔액이 가장 낮은 경로를 선택
   - 이로써 2000~2002, 2008, 2022 같은 하락장과 횡보장 순서위험이 미래 모드에도 반영됩니다.
...
- 미래 모드에서 단순 CAGR을 매년 반복하는 방식으로 되돌리지 마십시오. 그 방식은 하락장과 순서위험을 숨깁니다.
