...
3. DC 수익률에 보수적 비용 차감을 반영했습니다.
   - 안전자산 비용 차감: 연 0.10%p
   - 위험자산 비용 차감: 연 0.70%p
   - 이 값은 ETF 총보수, 기타비용, 추적오차, 거래비용, 환헤지·환전 관련 비용이 결과를 깎는 효과를 보수적으로 반영하기 위한 임시 상수입니다.
   - 실제 상품 연동 시 상품별 TER, 기타비용, 추적차이, 환헤지 비용으로 교체해야 합니다.
...
5. 미래 시뮬레이션을 평균 시나리오와 최악 시나리오로 분리했습니다.
   - 평균 시나리오: 선택 지수의 전체 기간 CAGR을 매년 반복하되, 비용 차감과 연말 납입 가정은 유지합니다.
...
- `web/src/lib/calc.ts`: DC 비용 차감, 연말 납입, 보수적 미래 경로, IRP 세금 로직
