...
미래 시뮬레이션 **Monte Carlo 확률 밴드** 추가 (상세 [DEVLOG 2026-07-01](DEVLOG/2026-07-01.md)):
- REVIEW Top10 **#4(미래 '평균' 확정수익 오인)의 근본 해법**. 지난 세션의 경고배너·과거빈도%는 완화였고, 이번은 분포 자체를 노출.
- `calc.ts` **`monteCarloBands()`**: 지수 실제 연도별 수익률을 IID 부트스트랩(1,500경로)·안전30/위험70+실부담비용 블렌딩 → p10/25/50/75/90 경로 + `probDcWins` + 종말 10~90%. **결정적(mulberry32 고정시드)**·이력<5년 null. `simulate()` `"band"` 분기(중앙값이 기존 verdict/세금 UI 구동, null이면 평균 폴백).
- UI: 미래 3버튼 **평균/확률 밴드/최악**(밴드 기본값), 불확실성 콘(10~90%+25~75% 띠)·중앙값선, "N% 확률로 DC 우위 · 10~90% A~B", 방법 캡션, 호버 3줄, `scn=band` 공유.
- 검증: tsc 클린 · vitest **42/42**(+7) · build 통과 · 프리뷰 E2E(밴드/평균/최악 무회귀·호버·모바일 오버플로0·다크). 콘솔 0.
- ⚠️ **밴드 중앙값(7.3%) > 평균(6.3%)는 버그 아님** — 평균은 지수 기하CAGR을 상수 적용, 밴드는 연도별 재추출 복리라 안전30%가 변동성 드래그를 줄여 실현수익↑. (DEVLOG "알아둘 점")
