...
미래 Monte Carlo **확률 밴드 추가 → 같은 세션에 제거** (상세 [DEVLOG 2026-07-01](DEVLOG/2026-07-01.md)):
- 구현·검증(vitest 42/42)까지 마쳤으나 **사용자 판단: "확률 밴드 개념이 일반 사용자에게 어렵다" → 제거**. 커밋 `7efba7c`를 `git revert`로 원복.
- 현재: 미래 시나리오 **평균/최악 2종(기본 최악)**, tsc 클린·**vitest 35/35**·build·프리뷰 무회귀.
- ⚠️ **재오픈**: REVIEW #4(미래 '평균' 직선 = 확정수익 오인)은 다시 완화(경고배너+과거빈도%)만 남음. 다시 다룬다면 **통계 개념 없이 쉬운 표현**으로.
